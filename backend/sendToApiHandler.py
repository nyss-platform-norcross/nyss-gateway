    ...
    while (True):
        ...
        if (len(fileList) is not len([name for name in os.listdir('./' + MESSAGES_OUTBOX_FOLDER)])):
            fileList = [name for name in os.listdir(
                './' + MESSAGES_OUTBOX_FOLDER)]
            getSmsAndPostToPlatform()


def getSmsAndPostToPlatform():
    if (len([name for name in os.listdir('./' + MESSAGES_OUTBOX_FOLDER)]) > 0):
        fileNameLatestSms = sorted(
            [name for name in os.listdir('./' + MESSAGES_OUTBOX_FOLDER)])[-1]
        smsDict = getSmsDict(fileNameLatestSms)
        r = postSmsToPlatform(smsDict)
        if(r.status_code == 200):
            os.rename('./' + MESSAGES_OUTBOX_FOLDER + fileNameLatestSms,
                      './' + MESSAGES_SENT_FOLDER + fileNameLatestSms)
        else:
            os.rename('./' + MESSAGES_OUTBOX_FOLDER + fileNameLatestSms, './' +
                      MESSAGES_ERRORS_FOLDER + fileNameLatestSms + '-' + str(r.status_code))
